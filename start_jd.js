...
function openAndInto() {
    ...
    app.startActivity({
        action: "VIEW",
        data: 'openApp.jdMobile://virtual?params={"category":"jump","action":"to","des":"m","sourceValue":"JSHOP_SOURCE_VALUE","sourceType":"JSHOP_SOURCE_TYPE","url":"https://u.jd.com/xwRhCdC","M_sourceFrom":"mxz","msf_type":"auto"}'
    })
}
...
function timeTask() {
    ...
    while (c < 40) { // 0.5 * 60 = 20 秒，防止死循环
        if ((textMatches(/获得.*?金币/).exists() || descMatches(/获得.*?金币/).exists())) // 等待已完成出现
            break
        if ((textMatches(/已浏览|已达上限/).exists() || descMatches(/已浏览|已达上限/).exists())) { // 失败
